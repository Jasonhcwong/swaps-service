...
module.exports = (args) => {
  ...
  const p2shNestedAddr = fromOutputScript(p2shWrappedWitnessProg, ltctestnet);

  const refundHash = Buffer.from(refundPublicKeyHash, 'hex');

  const scriptPub = script.witnessPubKeyHash.output.encode(refundHash);
  const refundP2wpkhAddress = address.fromOutputScript(scriptPub, ltctestnet);
  ...
  return {
    ...
    p2wsh_address: address.fromOutputScript(witnessProgram, ltctestnet),
